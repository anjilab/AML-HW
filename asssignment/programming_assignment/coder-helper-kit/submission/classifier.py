...
def plot(err, epoch,type):
    ...
    print(np.array(range(epoch)).size, np.array(err).size)
...
class feature_extractor:
    ...
    def __init__(self, vocab, tokenizer):
        print('Constructor of Extractor')
...
class classifier_agent():
    ...
    def batch_feat_map(self, sentences):
        ...
        :return: the resulting feature marix in sparse.csc_array of shape d by m
        '''
        try:
...
class tfidf_extractor(feature_extractor):
    ...
    def tfidf_feature(self,sentence):
        ...
        # x = self.compute_word_idf(sentence)
        tokenize_one_review = tokenize(sentence)
        term_frequency = Counter(tokenize_one_review)
        row_ind = []
        col_ind = []
        data = []
        for word, count in term_frequency.items():
            if word in self.vocab_dict:
                row_ind.append(self.vocab_dict[word])
                col_ind.append(0)
                data.append(count * self.word_idf[word]) # count should be tf * idf
...
def compute_word_idf(train_sentences,vocab):
    ...
    for sentence in train_sentences:
        unique_words = set(tokenize(sentence))
        # word_doc_freq = Counter(unique_words)
    ...
    total_docs = len(train_sentences)
    word_idf = {word: np.log(1+total_docs / (1 + freq)) for word, freq in word_doc_freq.items()}
    word_idf_ordered = {word: word_idf[word] for word in vocab}
    return word_idf_ordered
